...
| PipeWire, natively | A stream in each direction passing both contract suites against a live graph, a device list from the registry, device events, which device is default out of the metadata object, each device's own volume off its node, one application's output recordable on its own, and the stream's own volume as a control. What `AudioBackends.open` returns on Linux. |
...
| Should the native backend go first on a machine that has both? | 13.9 | **Answered: yes, and it does.** The two report the same capabilities except the sample cache, which the graph has no equivalent of, so `AudioBackends.open` takes a set of needed capabilities and a consumer that uses the cache names it and gets the rung below. `-Dlibsound.backend=` still pins either. |
| Does `latencyNanos` returning zero mean unmeasurable or empty? | 4.7 | **Answered: neither, and the question had a false premise.** No backend here returns zero because it cannot measure. Reading all five, a zero is a sink that is not open, a queue with nothing in it, or a query the server did not answer. `isOpen` separates the first, and `Capability.TOTAL_LATENCY` separates the other two without a capability of its own: present, the device's share is there while the stream is, so a zero on an open sink is a transient refusal; absent, the number is the client's queue and a zero is an empty one. |

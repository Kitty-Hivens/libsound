...
| Windows audio (WASAPI) | Runs. Device enumeration, playback, playhead and volume execute on every push against a Windows JVM under wine -- which checks the ABI, not the hardware. The contract suite is a separate matter and has never executed: wine runs the audible check rather than the JUnit suite, so `WasapiSinkContractTest` skips on every row there is. The rules it asserts are the ones a clock rides on, and on Windows they are argued rather than measured. Real devices need [docs/TESTING.md](docs/TESTING.md). |
